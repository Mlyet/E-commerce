...
const SignUp = () => {
        ...
        if (validator.isEmail(data.account.username)) {
            ...
            if (response.status === 201) {
                ...
                if (response.status === 200) {
                    ...
                    setUser({ email: jwt.username, role: jwt.roles[0], id: jwt.id });
                    navigate("/");
                }
        ...
        if (validator.isEmail(data.email)) {
            ...
            if (response.status === 201) {
                ...
                if (response.status === 200) {
                    ...
                    // currentUser.email = jwt.username;
                    // currentUser.role = jwt.roles[0];
                    setUser({ email: jwt.username, role: jwt.roles[0] });
                    navigate("/");
                }
                console.log(response);
            }
        ...
        if (step === 1) {
                ...
                <>
                    <div className="rounded-md">
                        <div className="flex flex-col w-60 mx-10">
                            <h1 className="mt-10 lg:mt-40 text-white">Firstname</h1>
                            <input
            ...
            setNextStep(
                <>
                    <div className="rounded-md">
                        <div className="flex flex-col w-60 mx-10">
                            <h1 className="mt-10 lg:mt-32 text-white">Birthdate</h1>
                            <input
                                className="pl-3 my-4 bg-transparent border-solid border-white border-b-2 outline-none rounded-md text-white"
                                onInput={(
                                    e: React.ChangeEvent<HTMLInputElement>
            ...
            setNextStep(
                <>
                    <div className="rounded-md">
                        <div className="flex flex-col w-60 mx-10">
                            <h1 className="mt-10 lg:mt-72 text-white">Password</h1>
    ...
    return (
        ...
        <Header />
            <main>
                <div className="bg-[url('ugly.png')] bg-cover h-4/5  bg-no-repeat bg-center flex m-auto" style = {{width: "86vw"}}>
                    <div className="xxs:w-0sm:ml-48 lg:w-1/2 lg:ml-0 xxs:mt-5"></div>
                    <div className="bg-[#1818189a] lg:m-0 m-auto h-screen backdrop-blur-sm xxs:w-auto xxs:h-min xxs:mt-5 lg:w-1/2 lg:h-screen lg:mt-0 lg:ml-0">
                        {nextStep}
