    ...
    if(state.id === 1){
        if(state.email !==""){
            useEffect(() => {
                ...
                const get_user_id = async()=>{
                    const response = await axios.get(`https://127.0.0.1:8002/api/users/${email}`)
        ...
        useEffect(() => {
            ...
            const checkoutinseration = async()=>{
                const response = await axios.post("https://127.0.0.1:8002/api/paymentinseration", inseration_data)
        ...
        return(
            <div className="mt-11">
                <div className="bg-white rounded grid grid-rows-2 m-auto w-1/3 h-64 pb-10">
                    ...
                    <div className="justify-center items-center m-auto">
                        <p>{state.msg}</p>
                        <Link to={`/orderview`}><button className="px-7 py-2 rounded mt-2 bg-[#83ed84]">See My Order</button></Link>
                    </div>
                </div>
            </div>
        )
